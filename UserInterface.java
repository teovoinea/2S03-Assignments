...
public class UserInterface{
	...
	public void P2(){
		...
		try{
			...
			System.err.println(ex.getMessage()); //print error message
		}
	...
	public ArrayList<String> readUser(){
		...
		try (BufferedReader br = new BufferedReader(new FileReader("Users.txt"))) {
			...
			e.printStackTrace(); //print stacktrace of the error
		}
		...
		try{
			...
			if (num > 0){ //check if the number is greater than 0
				...
				try{ //another try to catch any exceptions
					...
					e.printStackTrace(); //handle and print stack trace
				}
			}
			else{ //bad input
